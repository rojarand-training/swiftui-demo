    ...
    private func runAnimation() {
        if dark {
            label.moveRight(by: 50)
                ...
                .then(.changeText(of: label, to: "Hello World", duration: 1.5))
                .start()
            ...
            label.moveLeft(by: 50)
                ...
                .then(.changeText(of: label, to: "What a suprise, it works as expected", duration: 1.5))
                .start()
        }
        dark.toggle()
    }
...
extension Animation {
    ...
    static func changeText(of label: UILabel, to text: String, duration: TimeInterval = 0.5) -> Animation {
        Transition(view: label, duration: duration) { view in
            if let label = view as? UILabel {
                label.text = text
            }
        }
...
extension UILabel {
    @discardableResult func changeText(to text: String, duration: TimeInterval = 0.5) -> Animation {
        .changeText(of: self, to: text, duration: duration)
    }
}
